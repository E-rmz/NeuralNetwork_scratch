...
def load_data():
   ...
   # data = loadmat('digit_recog.mat')
    ...
    randomlist = random.sample(range(0, 5000), 1500)
    for i in randomlist:
        train_dataTest.append(train_data[i])
        arrTest.append(arr[i])
    b = (train_dataTest,arrTest)
    for i in range (5000):
    ...
    training_inputs = [np.reshape(x, (400,1)) for x in c[0]]
    
    ## the required format for our neural network's input layer
    ## ---
    ##print("hello")
    ##print(type(training_inputs[0][0]))
    training_results = [vectorized_result(y) for y in c[1]]
    ## 
    ...
    #print((valid_data[0]))
    validation_inputs = [np.reshape(x, (400,1)) for x in c[0]]
    
    ## reshaping the validation inputs to 400x1 vector
    ## ---
    validation_data = zip(validation_inputs, c[1])
   
    
    ## test_data:
    test_inputs = [np.reshape(x, (400,1)) for x in b[0]]
    ## reshaping the test inputs to 400x1 vector
    ## ---
    test_data = zip(test_inputs, b[1])
    ## 
