...
def load_data():
    ...
    numbers =[]
    train_data = loadmat('digit_recog')['X']
    mat_output = loadmat('digit_recog')["Y"]
    #print (train_data[0])
    ...
    training_inputs = [np.reshape(x, (400,1)) for x in a[0]]
 
    ## the required format for our neural network's input layer
    ## ---
    ##print("hello")
    ##print(type(training_inputs[0][0]))
    training_results = [vectorized_result(y) for y in a[1]]
    ## 
    ...
    validation_inputs = [np.reshape(x, (400,1)) for x in a[0]]
    
    ## reshaping the validation inputs to 784x1 vector
    ## ---
    validation_data = zip(validation_inputs, a[1])
   
    
   
    test_inputs = [np.reshape(x, (400,1)) for x in a[0]]
    ## reshaping the test inputs to 784x1 vector
    ## ---
    test_data = zip(test_inputs, a[1])
